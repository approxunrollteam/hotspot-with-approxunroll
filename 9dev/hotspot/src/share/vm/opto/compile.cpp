...
_has_reserved_stack_access(target->has_reserved_stack_access()) {
    ...
    if (target->approximated() ) {
        //Put verbosity options here later
        tty->print_cr("Compile::Compile - Method can be approximated %s", target->name()->as_quoted_ascii());
    } /*else {
        tty->print_cr("Compile::Compile - Method cannot be approximated %s", target->name()->as_quoted_ascii());
    }*/
...
void Compile::Init(int aliaslevel) {
    ...
    if (AllowVectorizeOnDemand) {
        if (has_method() && (_directive->VectorizeOption || _directive->VectorizeDebugOption)) {
            ...
            NOT_PRODUCT(if (do_vector_loop() && Verbose) {
    ...
    NOT_PRODUCT(if (use_cmove() && Verbose && has_method()) {
    ...
    switch (_AliasLevel) {
        ...
        case 1: // Flatten to: oop, static, field or array
            switch (tj->base()) {
                ...
                case Type::RawPtr: tj = TypeRawPtr::BOTTOM;
                    break;
                case Type::AryPtr: // do not distinguish arrays at all
                case Type::InstPtr: tj = TypeInstPtr::BOTTOM;
                    break;
                case Type::KlassPtr: tj = TypeKlassPtr::OBJECT;
                    break;
                case Type::AnyPtr: tj = TypePtr::BOTTOM;
                    break; // caller checks it
...
struct Final_Reshape_Counts : public StackObj {
    ...
    void inc_float_count() {
        _float_count++;
    }

    void inc_double_count() {
        _double_count++;
    }

    void inc_java_call_count() {
        _java_call_count++;
    }
    ...
    int get_float_count() const {
        return _float_count;
    }

    int get_double_count() const {
        return _double_count;
    }

    int get_java_call_count() const {
        return _java_call_count;
    }

    int get_inner_loop_count() const {
        return _inner_loop_count;
    }
};
...
void Compile::ConstantTable::emit(CodeBuffer& cb) {
    ...
    for (int i = 0; i < _constants.length(); i++) {
        ...
        switch (con.type()) {
            case T_LONG: constant_addr = _masm.long_constant(con.get_jlong());
                break;
            case T_FLOAT: constant_addr = _masm.float_constant(con.get_jfloat());
                break;
            case T_DOUBLE: constant_addr = _masm.double_constant(con.get_jdouble());
                break;
            case T_OBJECT:
            {
...
Compile::Constant Compile::ConstantTable::add(MachConstantNode* n, MachOper* oper) {
    ...
    switch (type) {
        case T_LONG: value.j = oper->constantL();
            break;
        case T_FLOAT: value.f = oper->constantF();
            break;
        case T_DOUBLE: value.d = oper->constantD();
            break;
        case T_OBJECT:
        case T_ADDRESS: value.l = (jobject) oper->constant();
            break;
        case T_METADATA: return add((Metadata*) oper->constant());
            break;
...
CloneMap& Compile::clone_map() {
    return _clone_map;
}

void Compile::set_clone_map(Dict* d) {
    _clone_map._dict = d;
}
