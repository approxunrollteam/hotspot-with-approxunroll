...
Node** DegradedUnrollingKit::find_array_storages(IdealLoopTree *loop) {
    ...
    if ( !head->is_CountedLoop() ) return NULL;
...
#ifdef TRACE_DEGRADE 
void DegradedUnrollingKit::print_loop(IdealLoopTree* loop) {
...
//******************************************************************************
//Marks the loop as approximate 

void DegradedUnrollingKit::mark_loop_as_approximated(IdealLoopTree *loop) {
    CountedLoopNode *head = loop->_head->as_CountedLoop();
    if ( head->is_CountedLoop() ) head->set_approximate(true);
}
...
bool DegradedUnrollingKit::approximate(IdealLoopTree *loop) {
...
#ifdef TRACE_DEGRADE     
    tty->print_cr("=====================");
    tty->print_cr("BEFORE APPROXIMATION:");
    print_loop(loop);
    tty->print_cr("=====================");
    tty->print_cr("ABOUT TO LOOK FOR STORAGES");
#endif
    mark_loop_as_approximated(loop);
    ...
    while (progress) {
        ...
        while (k < loop->_body.size()) {
            proj_progress = false;
            //If the node is an projection that can be approximate:
    ...
    tty->print_cr("=====================");
    tty->print_cr("AFTER APPROXIMATION:");
    print_loop(loop);
    tty->print_cr("=====================");
