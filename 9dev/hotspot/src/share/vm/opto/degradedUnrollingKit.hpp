...
private:
    ...
    //Marks the loop as it has been already approximated so the approximation
    //algorithm is not repeated anymore. This does not meant that the approximation
    //was successful, just that it was performed.
    void mark_loop_as_approximated(IdealLoopTree *loop);
    
#ifdef TRACE_DEGRADE 
    void print_loop(IdealLoopTree* loop);             
    //Tracing purposes
    uint level;        
#endif
...
public:
    ...
    bool approximate(IdealLoopTree *loop);
};
