...
bool IdealLoopTree::policy_unroll(PhaseIdealLoop *phase) {
    ...
    for (uint k = 0; k < _body.size(); k++) {
        Node* n = _body.at(k);
        switch (n->Opcode()) {
            case Op_XorI: xors_in_loop++;
                break; // CRC32 java code
            case Op_ModL: body_size += 30;
                break;
            case Op_DivL: body_size += 30;
                break;
            case Op_MulL: body_size += 10;
                break;
            case Op_StrComp:
            case Op_StrEquals:
            case Op_StrIndexOf:
    ...
    //DOUBLING UNROLLING BECAUSE OF THE APPROXIMATION: 
    
    
    int k = _local_loop_unroll_limit;
#ifdef TRACE_DEGRADE     
    if ( phase->C->method()->approximated() ) {
        tty->print_cr("Unrolling rules have changed");
        k *= 2;
    }
#endif 
    if (body_size > (uint) k) {
...
void PhaseIdealLoop::do_unroll(IdealLoopTree *loop, Node_List &old_new, bool adjust_min_trip) {
    ...
    clone_loop(loop, old_new, dd, NULL);
...
bool IdealLoopTree::iteration_split_impl(PhaseIdealLoop *phase, Node_List &old_new) {
    ...
    //Approximate the loop only if no previous attempt have made, and when the
    //loop has been already unrolled
    if ( !cl->is_approximate() && cl->unrolled_count() == 2 && 
            phase->C->method()->approximated()) {
        DegradedUnrollingKit degraded;
        //If the loop can be approximated, approximate it
        if (degraded.approximate(this)) {
            //Send the method to the Ideal Graph Visualization tool
            phase->C->print_method(PHASE_TRACE_POINT_2, 1);
